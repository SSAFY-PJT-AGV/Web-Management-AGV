    ...
    <svg viewBox="0 0 900 850" class="h-full w-full">
      ...
      <g :transform="`translate(${mapGroupX} ${mapGroupY}) scale(${mapScale})`">
        ...
        <g :transform="`translate(${storageGroupX} ${storageGroupY})`">
          <text x="0" y="0" text-anchor="middle" fill="#F8FAFC" font-size="20" font-weight="900">
            완제품 상자 보관 구역
          </text>
          ...
          <text x="0" y="280" text-anchor="middle" fill="#F8FAFC" font-size="20" font-weight="900">
            자재 상자 보관 구역
          </text>
...
function getAgvCurrentMarkerId(agv) {
  return agv?.currentMarkerId ?? agv?.current_marker_id ?? agv?.located ?? null
}

function getAgvMarker(agv) {
  const markerId = getAgvCurrentMarkerId(agv)

  if (markerId === null || markerId === undefined) {
    return null
  }

  return props.markerById(Number(markerId)) ?? props.markerById(String(markerId)) ?? null
}

function getAgvX(agv) {
  const marker = getAgvMarker(agv)
